...
const BuyXGetY = () => {
    ...
    useEffect(() => {
        if (staff.permissions?.includes('buyxgety_manage')) {
    ...
    const handleEdit = async (data) => {
        ...
        handleBrandSelection({ label: data.brand_id.full_name, value: data.brand_id._id });
        ...
        if (selectedOutlet) {
            ...
            }, { label: data.brand_id.full_name, value: data.brand_id._id });
        } else {
        ...
        const _buyItem = items.find(i => i._id ===  data.buy_item._id);
        const _getItem = items.find(i => i._id ===  data.get_item._id);
        if(_buyItem && _getItem){
            handleItemSelection({label: _buyItem.name, value : _buyItem._id}, "buy");
            handleItemSelection({label: _getItem.name, value : _getItem._id}, "get");
        }
    ...
    const filteredData = useFilteredData({
        data: data,
        searchTerm: search,
        searchKeys: ["name", "status", "day", "start_time", "end_time", "brand_id.short_name", "outlet_id.name", "menu_id.name", "buy_item.name", "get_item.name"],
        filters: {
            status: filterStatus,
        },
    });
    ...
    return (
        <>
            ...
            }
            {
                showPopup ?
                    <div className='card'>
                        <HeadingText title={`${isEditing ? "Edit" : "Add"} Buy X Get Y Item`} />
