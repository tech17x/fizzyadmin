    ...
    return (
        <>
            ...
            }
            {
                showPopup ?
                    <div className='card'>
                        <HeadingText title={`${isEditing ? "Edit" : "Add"} Customer`} />
                    ...
                    showBrandSelectionForBulkInsertion ?
                        <div className='card'>
                            <HeadingText title={`Select Brand`} />
                            <div className="inputs-container">
                                <div className="inputs-row">
                                    <SelectInput
                                        label="Brand"
                            ...
                            <div className="action-btns-container">
                                ...
                                <Button clickAction={() => { setSelectedBrand(null); setSelectedOutlet(null); setBulkData([]); setSelectedFile(""); setSelectedFileName('Select File'); setShowBrandSelectionForBulkInsertion(false) }}>Close</Button>
                            </div>
                        </div> :
                        showSMSModel ?
                            ...
                            : orderSummaryPopup ?
                                (
                                    <div className="order-summary-popup card">
                                        <HeadingText title={`Order Summary for ${selectedCustomer.name}`} />
                                        <div>
                                            <p><b>Name:</b> {selectedCustomer.name}</p>
                                            <p><b>Note:</b> {selectedCustomer.note || 'N/A'}</p>
                                            <p><b>Phone:</b> {selectedCustomer.phone || "NA"}</p>
                                            <p><b>Email:</b> {selectedCustomer.email || "NA"}</p>
                                            <p><b>Total Orders:</b> {orderStats?.totalOrders ?? "NA"}</p>
                                            <p><b>Total Revenue:</b> ₹{orderStats?.totalRevenue ?? "NA"}</p>
                                            <p><b>Last Visited:</b> {orderStats?.lastVisitedOn ?? "NA"}</p>
                                            <p><b>Average Order Value:</b> ₹{orderStats?.averageOrderValue ?? "NA"}</p>
                                            <p><b>Order Frequency:</b> {orderStats?.ordersPerMonth ?? "NA"} orders/month</p>
                                            <p><b>Most Loved Item:</b> {orderStats?.mostLovedItem ?? "NA"}</p>
                                        </div>
                                        <div>
                                            <b>Recent Orders:</b>
                                            <table>
                                        ...
                                        <div className="action-btns-container">
                                            <Button clickAction={() => setOrderSummaryPopup(false)}>Close</Button>
                                        </div>
                                    </div>
                                ...
                                <div className="table-section-container" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                                    ...
                                    <div className="add-new-staff-info card">
                                        ...
                                        <div className="right-shift" style={{ display: "flex", gap: '1rem', flexWrap: 'wrap' }}>
                                            <GradientButton clickAction={() => handleAddData()}>Add Customer</GradientButton>
                                            <GradientButton clickAction={() => handleDownloadCustomerSample()}>Format</GradientButton>
                                                            ...
                                                            <tr key={index}>
                                                                ...
                                                                <td>
                                                                    <div className="tax-action-btns">
                                                                        ...
                                                                        <button onClick={() => fetchCustomerOrders(item)}>
