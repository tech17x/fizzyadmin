...
const Orders = () => {
    ...
    // 🔹 Items (assuming you want to manage a list of items with quantity, price, and addons)
    ...
    // To temporarily hold selected item (used before pushing to itemsList)
    ...
    const [selectedExtraCharge, setSelectedExtraCharge] = useState(null);
    const [extraChargeAmount, setExtraChargeAmount] = useState(0);

    // 🔹 Table & Floor (only for dine-in)
    const [selectedTable, setSelectedTable] = useState(null);
    const [selectedFloor, setSelectedFloor] = useState(null);

    // 🔹 Status
    const [orderStatus, setOrderStatus] = useState({ label: "Pending", value: "pending" });
    const [paymentStatus, setPaymentStatus] = useState({ label: "Pending", value: "pending" });

    // 🔹 Notes
    const [notes, setNotes] = useState("");

    // 🔹 Totals (optional, may be computed)
    const [subtotal, setSubtotal] = useState(0);
    const [totalAmount, setTotalAmount] = useState(0);

    useEffect(() => {
        if (staff.permissions?.includes('orders_view')) {
    ...
    const handleEditAddon = (addon) => {
        ...
        setAllItem(addon.status === "active" ? true : false);
        setAddonStatus(addon.status === "active" ? true : false);
        const selectedBrand = brands.find(brand => brand._id === (addon.brand_id?._id || addon.brand_id));
    ...
    const handleSave = async () => {
        setLoading(true);
        if (!selectedBrand || !name || (!selectedOutlet) || (!selectedMenu) || !addonPrice) {
    ...
    const handleOutletSelection = (outlet) => {
        if (outlet) {
            ...
            fetchMenus(selectedBrand, outlet);
        }
    }
    ...
    return (
        <>
            ...
            }
            {
                showPopup ?
                    <div className='card'>
                        <HeadingText title={`${isEditing ? "Edit" : "Add"} Order`} />
                        <div className="inputs-container">
                            {/* Brand, Outlet, Customer */}
                            ...
                            {/* Items - you can make this a dynamic list */}
                            <div className="inputs-row">
                                <SelectInput
                                    label="Payment Type"
                                    selectedOption={selectedPaymentType}
                                    onChange={setSelectedPaymentType}
                                // options={paymentTypes.map(p => ({ label: p.name, value: p._id }))}
                                />
                                <SelectInput
                                    label="Item"
                            ...
                            <div className="inputs-row">
                                <SelectInput
                                    ...
                                    options={addons.map(a => ({ label: a.name, value: a._id }))}
                                    isMulti
                                />
                                <SelectInput
                                    label="Discount"
                            ...
                            <div className="inputs-row">
                                ...
                                <SelectInput
                                    label="Extra Charge"
                                    selectedOption={selectedExtraCharge}
                                    onChange={setSelectedExtraCharge}
                                // options={extraCharges.map(e => ({ label: e.name, value: e._id }))}
                                />
                            </div>
                            <div className="inputs-row">
                            ...
                            {/* Table & Floor (only for Dine-in) */}
                            {
                                selectedOrderType?.label?.toLowerCase() === "dine-in" && (
                                    <div className="inputs-row">
                                        <SelectInput
                                            label="Table"
                                            selectedOption={selectedTable}
                                            onChange={setSelectedTable}
                                        // options={tables.map(t => ({ label: t.name, value: t._id }))}
                                        />
                                        <SelectInput
                                            label="Floor"
                    ...
                    <div className="table-section-container" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                        ...
                        <div className="add-new-staff-info card">
                            {/* <GradientButton clickAction={handleAddNewAddon}>Add Order</GradientButton> */}
                            ...
                            <NoDataFound message='No order history found !'/>
                        </div>
                    </div>
            }
        </>
