...
export const AuthProvider = ({ children }) => {
    ...
    const fetchStaff = useCallback(async () => {
        ...
        try {
            const response = await axios.get("http://localhost:5001/validate_token", { withCredentials: true });
    ...
    // Logout function
    const logout = async () => {
        await axios.post("http://localhost:5001/api/staff/logout", {}, { withCredentials: true });
        setStaff(null);
    };
